...
static hash_val_t pair_hash(const void *key) {
    register struct state *const *pair = (struct state *const *) key;
...
static struct state_set *fa_reverse(struct fa *fa) {
    ...
    for (int i=0; i < all->used; i++) {
        ...
        struct trans *t = (struct trans*)all->data[i];
        s->accept = 0;
...
static struct state_set *state_set_hash_uniq(state_set_hash *smap,
    ...
    const struct state_set *orig_set = (struct state_set*)hnode_getkey(node);
    if (orig_set != set) {
        state_set_free(set);
    }
...
static hash_val_t set_hash(const void *key) {
    hash_val_t hash = 0;
    const struct state_set *set = (struct state_set*)key;
...
static int set_cmp(const void *key1, const void *key2) {
    const struct state_set *set1 = (struct state_set*) key1;
    const struct state_set *set2 = (struct state_set*) key2;
...
static struct fa *fa_clone(struct fa *fa) {
    ...
    for (int i=0; i < set->used; i++) {
        ...
        struct state *sc = (struct state*)set->data[i];
        for_each_trans(t, s) {
            int to = state_set_index(set, t->to);
            assert(to >= 0);
            struct state *toc = (struct state*)set->data[to];
...
int fa_example(struct fa *fa, char **example, size_t *example_len) {
    ...
    *example_len = 0;
    ...
    while (worklist->used) {
        struct state *s = state_set_pop(worklist);
        struct re_str *ps = (struct re_str*)state_set_find_data(path, s);
        for_each_trans(t, s) {
            char c = pick_char(t);
            ...
            if (toind == -1) {
                ...
                path->data[toind] = string_extend((struct re_str*)path->data[toind], ps, c);
            }
        }
    }
    ...
    for (int i=0; i < path->used; i++) {
        ...
        struct re_str *ps = (struct re_str*)path->data[i];
        if (p->accept &&
...
static int re_concat_as_string(const struct re *re, struct re_str *str) {
    ...
    for (int i=0; i < nre; i++) {
        ...
        if (re_needs_parens_in_concat(res[i]))
            *p++ = '(';
        p = (char*)memcpy(p, strings[i].rx, strings[i].len);
        p += strings[i].len;
        if (re_needs_parens_in_concat(res[i]))
            *p++ = ')';
    }
